...
sms['amount'] = sms['content'].str.extract(r'(?:Amount:\s*Tk|amount)\s*([\d,]+)(?:\.\d{2})?', expand=False).str.replace(',', '', regex=False).replace('', pd.NA).astype(float).astype('Int64')

sms_in_max_debit_amount = np.max(sms.loc[(sms.type==1)&((sms.phone.str.contains('bKash',case = False, na = False))|(sms.phone.str.contains('nagad',case = False, na = False)))&(sms.content.str.contains(debit_words,case= False,na= False))]['amount'])
# sms_in_d30_min_bkash_credit_amount if pd.notna(sms_in_d30_min_bkash_credit_amount) else -1
